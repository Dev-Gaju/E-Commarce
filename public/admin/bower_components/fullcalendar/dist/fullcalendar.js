...
// like computeGreatestUnit, but has category abilities to interpret the source input for clues
...
but rather, a "category" token that has custom rendering (see specialTokens map).
*/
...
/*
Formats `date` with a Moment formatting string, but allow our non-zero areas and category token
*/
...
function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
	...
	for (
		...
		// If current chunk is on the boundary of unique date-content, and is a category-case
...
- fakeFormatString: a momentJS formatting string, littered with category control characters that get post-processed.
...
/*
Potentially splits a literal-text string into multiple parts. For category cases.
*/
...
/*
Given chunks parsed from a real format string, generate a fake (aka "intermediate") format string with category control
	...
	for (i = 0; i < fakeParts.length; i++) {
		...
		if (fakePart.charAt(0) === SPECIAL_TOKEN_MARKER) {
			parts.push(
				...
				// call category token's registered function.
...
function getDraggedElMeta(el) {
	...
	if (eventProps) {
		...
		// pluck category-cased date/time properties
...
Calendar.mixin({
	...
	setOptions: function(newOptionHash) {
		...
		// category-case handling of single option change.
...
var MonthView = FC.MonthView = BasicView.extend({
	...
	// Overrides the default BasicView behavior to have category multi-week auto-height logic
	setGridHeight: function(height, isAuto) {
